...
import { RaceEditorComponent } from './race-editor';
import { FeatEditorComponent } from './feat-editor';
import { ImportPanelComponent } from './import-panel';

type DevTab = 'classes' | 'subclasses' | 'pools' | 'races' | 'feats' | 'backgrounds' | 'spells' | 'import';

@Component({
  selector: 'app-developer',
  standalone: true,
  imports: [
    ClassEditorComponent, SubclassEditorComponent, PoolEditorComponent, BackgroundEditorComponent,
    SpellEditorComponent, RaceEditorComponent, FeatEditorComponent, ImportPanelComponent
  ],
  template: `
    <div class="page">
      <h1 class="mb-1">⚒ Gestione Contenuti</h1>
      <p class="muted mb-2">Sezione Developer: gestisci classi, sottoclassi, abilità, razze, sottorazze, talenti, background, incantesimi e pool di opzioni. Le modifiche sono salvate nel repository dei contenuti.</p>
      ...
      <div class="tabs">
        ...
        <button type="button" class="tab" [class.active]="tab() === 'races'" (click)="tab.set('races')">Razze</button>
        <button type="button" class="tab" [class.active]="tab() === 'feats'" (click)="tab.set('feats')">Talenti</button>
      ...
      @switch (tab()) {
        ...
        @case ('races') { <app-race-editor class="fade-in" /> }
        @case ('feats') { <app-feat-editor class="fade-in" /> }
